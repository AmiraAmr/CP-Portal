...
class laborerattendingController extends Controller
{
    ...
    public function sortUsersByProject(project $project)
    {
        ...
        $project = $project->contract()->select(['user_id', 'id', 'project_id'])->with(['user' => function ($q) {
    ...
    public function attendance_Manule()
    {
        ...
        return Inertia::render('Management/ManualAttendance/Index', ['projects' => $projects]);
        ...
        if (is_string($User)) {
            ...
            if (!empty($Attending_and_leaving)) {
                ...
                if (!empty($check->project) && $check->project->id !== null) {
                    ...
                    if ($User_overall) {
                        $n = $time / 540 > 1 ? 1 : $time / 540;

                        $User_overall->increment('num_of_attendance', $n);
                        ...
                        $User_overall->increment('time_attendance', $time > 540 ?  540 : $time);

                        $User_overall->increment('overtime', $time > 540 ? $time - 540 : 0);
                    } else {
                        ...
                        project_overall::create([
                            ...
                            'performance_point' => 0,
                            'time_attendance' => $time > 540 ?  540 : $time,
